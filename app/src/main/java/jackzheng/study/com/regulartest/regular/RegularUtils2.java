...
//            }
    ...
    private static void creatLastData(DateBean2 bean){
        ...
        int count =0;
        if(bean.local.size() == bean.mCountList.size()){
            for(Integer item : bean.mCountList){
                count+=item;
            }
        }else if(bean.mCountList.size() == 1){
            count = bean.local.size()* bean.mCountList.get(0);
        }
        count = count*bean.mLastData.size();
        bean.allCount = count;
        return ;
    }
